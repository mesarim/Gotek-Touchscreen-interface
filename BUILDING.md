...
_Firmware: **Gotek_JC3248 v4.8.4** — OMEGAWARE. Target board: **Guition JC3248W535C** (ESP32-S3-N16R8, 16 MB flash / 8 MB PSRAM, AXS15231B QSPI display + touch)._
...
|---|---|---|
| **JPEGDEC** (Larry Bank / bitbank2) | Arduino **Library Manager** → search `JPEGDEC` | Decoding cover-art / screensaver **JPG** images |
| **PNGdec** (Larry Bank / bitbank2) | Arduino **Library Manager** → search `PNGdec` | Decoding cover-art / screensaver **PNG** images (added v4.8.4). Same author as JPEGDEC. Both headers define `INTELSHORT`/`MOTOLONG` etc.; the sketch `#undef`s them between the two `#include`s, so redefinition warnings are already handled — no action needed. |
