...
// ============================================================================
// 5.4.0-7IN: accessibility font scale (FONT=SMALL/NORMAL/LARGE). Declared here so the
...
public:
  ...
  void setFont(const uint8_t* f){
    ...
    fscale*=g_font_mult;   // 5.4.0-7IN: FONT=SMALL/NORMAL/LARGE accessibility scale
  }
...
static void relayout(){
  ...
  if(!g_portrait_mode){
    ...
    COVER_W=300; COVER_H=LCD_HEIGHT-STATUS_H-BOTTOM_H;   // 5.4.0-7IN: +50% cover (room on the big panel)
    ...
    ITEMS_VIS=(int)(8.0f/g_font_mult+0.5f); if(ITEMS_VIS<3)ITEMS_VIS=3;   // 5.4.0-7IN: fewer/taller rows on LARGE
    LIST_ITEM_H=(LIST_BOTTOM-LIST_TOP)/ITEMS_VIS;
  } else {
    // ── Portrait (1/3): cover full-width on top, list full-width below ──
    COVER_W=LCD_WIDTH; COVER_H=270;   // 5.4.0-7IN: +50% cover
...
static const Theme THEMES[] = {
  ...
  { "GOLD",
    ...
    0x3200, 0xFCC0 },
  // ── 5.4.0-7IN high-vis accessibility themes ──
...
static void applyFont(int f){   // 5.4.0-7IN
