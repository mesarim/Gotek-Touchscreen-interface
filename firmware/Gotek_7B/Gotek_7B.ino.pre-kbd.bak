...
// v4.14.13-7B: recurse image-less "organisational" folders and flatten their games into
// the single alphabetical list (screen identical, card just tidy). A child dir with NO direct
// disk images is organisational, so we recurse into it — games nested under letter/label
// folders (ADF/A/<game>/, ADF/Games/<game>/) still surface flat. 7B has no Categories mode.
static void scanDirImages(const String& dir, std::vector<String>& out, const String& ext1, int depth){
  ...
  while ((gameDir = root.openNextFile())) {
    ...
    if (gameDir.isDirectory()) {
      ...
      if(out.size()==_imgs0){                            // image-less child = organisational -> recurse
        scanDirImages(entryName, out, ext1, depth+1);
      }
    } else {
...
static bool listImages(fs::FS& fs, std::vector<String>& out) {
  ...
  // folders, or flat /ADF/<file>.adf — image-less folders are recursed into and flattened.
...
static void  doScanDongles();   // v4.8.17: full-screen dongle scan + picker (JC method)
...
static int LCD_WIDTH=1024, LCD_HEIGHT=600;
...
static void doRescan(){
  ...
  // v4.13.0-7B: re-read CONFIG.TXT on a RESCAN so card edits (theme, categories, font,
...
static void doPairNow(){ doScanDongles(); }   // v4.8.17: pairing now goes through the scan+picker
...
static void drawBottomBar(){ UiFrame _uf;
  ...
  const int nb=5; int bw=LCD_WIDTH/nb;   // v4.8.16-7B: enlarged word-labelled rounded-rect nav keys (was small circle glyphs)

  const uint16_t bcol[5]={COL_ORANGE,COL_BLUE,COL_AMBER,COL_GREEN,COL_MID};
  const char* blbl[5]={"< PREV","NEXT >","THEME","GRID","INFO"};
  ...
  for(int i=0;i<nb;i++){
    ...
    int tw=UG->textWidth(blbl[i]);
    if(i==3){
      drawCarouselIcon(rx+16, ry+rh/2, col);                 // keep the coverflow glyph on REEL
      int wax=rx+30, waw=rw-30-4;
      UG->setCursor(wax+(waw-tw)/2, ry+(rh-12)/2); UG->print(blbl[i]);
    } else {
      UG->setCursor(rx+(rw-tw)/2, ry+(rh-12)/2); UG->print(blbl[i]);
    }
  }
...
static void drawCarousel(){ UiFrame _uf;   // static 3x2 GRID page (no motion) — replaces the carousel
  ...
  if(n==0){
    ...
    for(int c=0;c<GRID_PER;c++){
      ...
      if(isSel||isLd){                                   // INSERT/EJECT chip = the deliberate mount button
        const char*chip=isLd?"EJECT":"INSERT";
        UG->setFont(&lgfx::fonts::DejaVu9);
  ...
  GBTN(0,COL_ORANGE,String("LIST"));
  GBTN(1,COL_BLUE,String("< PREV"));
  GBTN(2,COL_MID,pg);
  GBTN(3,COL_GREEN,String("NEXT >"));
  #undef GBTN
}
...
static void gridHandleTap(uint16_t px,uint16_t py){
  ...
  if(py>=(uint16_t)(H-BOTTOM_H)){                         // bottom bar quarters
    ...
    else if(px<(uint16_t)(3*q)) { /* no source cycle on 7B */ }   // mid cell
...
// ── Full-screen dongle scan + picker (v4.8.17: ported from the JC method) ─────
...
static void doScanDongles(){
  ...
  if(n==0){
    UG->fillScreen(COL_BG);
    UG->setFont(&lgfx::fonts::DejaVu18); UG->setTextColor(COL_ORANGE,COL_BG);
    UG->setCursor(24,44); UG->print("No dongles found");
    UG->setFont(&lgfx::fonts::DejaVu12); UG->setTextColor(COL_DIM,COL_BG);
    UG->setCursor(24,92); UG->print("Check dongle is powered");
    UG->setCursor(24,118); UG->print("and in WIRELESS range.");
    ...
    UG->setCursor(bx+(bw-UG->textWidth("BACK"))/2,by+16); UG->print("BACK");
    uiFlush();
    uint32_t w=millis();
    ...
    if(dirty){ dirty=false;
      ...
      for(int r=0;r<maxRows&&(scanScroll+r)<n;r++){ int i=scanScroll+r, y=listTop+r*rowH;
        ...
        if(isActive){ UG->setFont(&lgfx::fonts::DejaVu12); UG->setTextColor(COL_GREEN,bg);
          UG->setCursor(LCD_WIDTH-28-UG->textWidth("ACTIVE"),y+18); UG->print("ACTIVE"); }
      }
      ...
      if(!moved){
        if((int)downY>=btnBarY){ int gap=14, bw=(LCD_WIDTH-4*gap)/3, i=((int)downX-gap)/(bw+gap);
          if(i==0){ espnowScanSelect(sel);
            ...
            { const char* s="PAIRED"; UG->setCursor((LCD_WIDTH-UG->textWidth(s))/2,LCD_HEIGHT/2-12); UG->print(s); }
